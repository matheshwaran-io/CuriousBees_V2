...
export default function CompactComposer({ onPostCreated }: CompactComposerProps) {
  ...
  return (
    <>
      ...
      <div className={`bg-white border-b border-slate-200/80 transition-all ${isFocused ? 'border-b-[#0C4DA2]/20' : ''}`}>
        <div className="flex gap-3 p-4 pb-0">
          ...
          <div className="flex-1 min-w-0">
            <textarea
              ...
              className="w-full bg-transparent border-none resize-none text-[15px] text-slate-900 font-medium placeholder:text-slate-400 placeholder:font-medium focus:outline-none min-h-[52px] leading-relaxed py-2.5"
              rows={1}
            />
                ...
                >
                  <div className="relative inline-block" ref={typeMenuRef}>
                    <button
                      ...
                      className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold border transition-all cursor-pointer ${activeType.color}`}
                    >
                    ...
                    <AnimatePresence>
                      {showTypeMenu && (
                        <motion.div
                          ...
                          className="absolute left-0 top-full mt-1 w-52 bg-white rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.12)] border border-slate-200/80 py-1.5 z-30"
                        >
                          {availableTypes.map(type => (
                            <button
                              ...
                              className={`w-full flex items-center gap-2.5 px-3.5 py-2 text-left text-xs font-bold transition-colors cursor-pointer ${
                                postType === type.id 
                                  ? 'bg-[#0C4DA2]/5 text-[#0C4DA2]' 
                                  : 'text-slate-700 hover:bg-slate-50'
                              }`}
                            >
                              <type.icon className="w-4 h-4 shrink-0" />
                              <span>{type.label}</span>
                              {postType === type.id && <Check className="w-3.5 h-3.5 ml-auto text-[#0C4DA2]" />}
                            </button>
                          ))}
                        </motion.div>
                      )}
            ...
            {/* Attachment Preview */}
            {attachment && (
              <div className="flex items-center gap-2 mb-2 bg-slate-50 border border-slate-200/80 rounded-xl px-3 py-2">
                <FileText className="w-4 h-4 text-[#0C4DA2] shrink-0" />
                <span className="text-xs font-bold text-slate-700 truncate flex-1">{attachment.name}</span>
                ...
                <button 
                  ...
                  className="text-slate-400 hover:text-rose-500 transition-colors cursor-pointer p-0.5 hover:bg-rose-50 rounded-full shrink-0"
                >
            ...
            <AnimatePresence>
              {isFocused && (postType === 'PUBLICATION' || isPaper) && (
                <motion.input
                  ...
                  className="w-full bg-transparent border-none text-xs text-slate-600 font-medium placeholder:text-slate-400 focus:outline-none mb-2 pb-2 border-b border-slate-100"
                />
              )}
            </AnimatePresence>
            ...
            <div className="flex items-center justify-between py-2.5 border-t border-slate-100">
              ...
              <div className="flex items-center gap-0.5">
                ...
                <button
                  ...
                  className="w-9 h-9 rounded-full flex items-center justify-center text-[#0C4DA2] hover:bg-[#0C4DA2]/10 transition-all cursor-pointer disabled:opacity-40"
                  title="Photo"
                >
                ...
                <button
                  ...
                  className={`w-9 h-9 rounded-full flex items-center justify-center transition-all cursor-pointer ${
                    postType === 'PUBLICATION' ? 'text-[#0C4DA2] bg-[#0C4DA2]/10' : 'text-[#0C4DA2] hover:bg-[#0C4DA2]/10'
                  }`}
                ...
                <button
                  ...
                  className="w-9 h-9 rounded-full flex items-center justify-center text-[#0C4DA2] hover:bg-[#0C4DA2]/10 transition-all cursor-pointer"
                  title="Add Hashtag"
                >
                ...
                <button
                  ...
                  className={`w-9 h-9 rounded-full flex items-center justify-center transition-all cursor-pointer ${
                    postType === 'COLLABORATION_REQUEST' ? 'text-[#0C4DA2] bg-[#0C4DA2]/10' : 'text-[#0C4DA2] hover:bg-[#0C4DA2]/10'
                  }`}
                ...
                <button
                  ...
                  className={`w-9 h-9 rounded-full flex items-center justify-center transition-all cursor-pointer ${
                    postType === 'ACHIEVEMENT' ? 'text-amber-500 bg-amber-50' : 'text-[#0C4DA2] hover:bg-[#0C4DA2]/10'
                  }`}
                  title="Achievement"
                >
                ...
                {isUploading && (
                  <div className="w-9 h-9 flex items-center justify-center">
                    <Loader2 className="w-4 h-4 text-[#0C4DA2] animate-spin" />
                  </div>
                )}

                {/* Character count (shown when typing) */}
                {charCount > 0 && (
                  <span className={`text-[10px] font-bold ml-1 ${isOverLimit ? 'text-rose-500' : charCount > 2200 ? 'text-amber-500' : 'text-slate-400'}`}>
                    {charCount}/{charLimit}
                  </span>
                )}
              </div>
              ...
              <button
                ...
                disabled={!canPost}
                className="px-5 py-2 bg-[#0C4DA2] hover:bg-[#0a3f8a] text-white text-[13px] font-extrabold rounded-full transition-all duration-200 shadow-sm hover:shadow-md active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer flex items-center gap-1.5"
              >
                {isSubmitting ? (
