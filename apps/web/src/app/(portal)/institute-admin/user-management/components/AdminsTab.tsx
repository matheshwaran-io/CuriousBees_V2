...
export default function AdminsTab() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const { apiMutate } = await import('@/lib/api-client');
      await apiMutate(`/api/admin/admins/${deletingAdmin.id}`, 'DELETE');
                    ...
                    <td className="px-4 py-3 text-right">
                      {!isSuperadmin ? (
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <button onClick={() => setDeletingAdmin(item)} className="p-1.5 hover:bg-red-100 text-red-600 rounded transition-colors cursor-pointer" title="Delete"><Trash2 className="w-4 h-4" /></button>
                        </div>
                      ) : (
      ...
      <AnimatePresence>
        {deletingAdmin && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm cursor-pointer" onClick={() => setDeletingAdmin(null)} />
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-sm bg-white rounded-2xl shadow-2xl p-6 border border-slate-100 flex flex-col items-center text-center z-10">
              ...
              <h3 className="font-display font-bold text-lg text-slate-900 mb-1">Delete Admin?</h3>
              <p className="text-xs text-slate-500 mb-6 leading-relaxed">
                Are you sure you want to delete <strong className="text-slate-800">{deletingAdmin.name || deletingAdmin.email}</strong>? This action will permanently remove their admin account.
              </p>
              <div className="flex gap-3 w-full">
                <button type="button" onClick={() => setDeletingAdmin(null)} disabled={deleting} className="flex-1 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl transition-colors cursor-pointer disabled:opacity-50">
                  Cancel
                </button>
